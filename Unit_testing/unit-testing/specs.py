...
def smallest_factor(n):
    ...
    for i in range(2, int((n**.5)+1)):
        #print("i is:", i, "n is :", n , "and result is",n % i)
        if n % i == 0: return i
    return n
...
def count_sets(cards):
    ...
    UniqueCards = True
    checkUniquenessCombination = combinations(cards,2)
    for i in checkUniquenessCombination:
        a,b = i
        if a is b:
            UniqueCards = False
            break

    #check that each string has only 4 characters
    rightLength = False
    lengthCheck = 0
    for i in cards:
        lengthCheck += len(i)
    ...
    #verify that each string has only characters "012"
    appropriateCharacters = ["0","1","2"]
    correctCards = True
    for i in cards:
        for j in range(4):
            #print(i)
            #print(i[j])
            if i[j] not in appropriateCharacters:
                correctCards = False
                #print("breaking")
                break

    #print("uncomment the exceptions")
    if len(cards) != 12:
        raise ValueError("there are not exactly 12 cards")
    if UniqueCards is False:
        raise ValueError("the cards are not all unique")
    if rightLength is False:
        raise ValueError("one or more cards does not have exactly 4 digits")
    if correctCards is False:
        raise ValueError("one or more cards has a character other than 0, 1, or 2")

    #print("checking number of sets")
    totalSets = 0
    chooseThreeCombinations = combinations(cards,3)
    for i in chooseThreeCombinations:
        isset = False
        a,b,c = i
        isset = is_set(a,b,c)
        if isset:
...
def is_set(a, b, c):
    ...
    #go three each of the characters in the cards and sum them
    #it's a set if each digit is congruent to zero modulo 3.
    for i in range(4):
...
if __name__ == "__main__":
    ...
    hand1 = ["1022", "1122", "0100", "2021",
                "0010", "2201", "2111", "0020",
                "1102", "0210", "2110", "1020"]
    total = count_sets(hand1)
    print(total)


    bool = is_set("1012","1022","1012") #it fails on the third digit
    bool = is_set("1022","1121","1220") #IS a SET
    if bool:
        print("it's true")
    else:
        print("it's false")
